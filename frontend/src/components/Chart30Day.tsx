...
const Chart30Day: React.FC<ChartProps> = ({ productName, history }) => {
  const now = new Date();
  ...
  return (
    <Card className="col-span-4 w-full">
      <CardHeader>
        <CardTitle>30-Day Price History: {productName}</CardTitle>
      </CardHeader>
      <CardContent className="pt-2">
        {filteredData.length === 0 ? (
          <p className="text-sm text-center text-gray-500 dark:text-gray-400">
            No price data found in the past 30 days.
          </p>
        ) : (
